...
class plgSystemJnilla extends JPlugin
{
	...
	public function onAfterInitialise() {
		...
		// Set development mode param
		if($isSuperAdmin){
			$jnHudDevelopmentMode = $input->get('jnHudDevelopmentMode');
			if(isset($jnHudDevelopmentMode)){
				if($jnHudDevelopmentMode == 1){
					plgSystemJnillaHelper::setDevelopmentModeParam(true);
				}else{
					plgSystemJnillaHelper::setDevelopmentModeParam(false);
					return false;
				}
			}
		}
