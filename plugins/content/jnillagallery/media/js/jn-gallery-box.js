//jn-gallery-box version 0.0.3

(function($)
{
	$(document).ready(function()
	{
		if(!$('.jn-gallery-box').length) return;
		// init jn-gallery-box
		var addClass;
		var target;
		var jnBox = $('<div id="jn-gallery-box" class="modal hide fade" tabindex="-1" role="dialog" aria-hidden="true"><button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button><div class="modal-body"></div></div>');
		jnBox.modal('hide');
		...
		// events
		$('.jnilla-gallery').on('click', '.jn-gallery-box', function(event){
			...
			if($(this).prop("tagName") == "A") {
				...
				if(/\.(?:jpg|jpeg|gif|png|svg)$/i.test(href)){
					var img = $('<img class="jn-gallery-box-img"/>');
					img.attr('src', href);
				...
				modalBody.html('<img class="jn-gallery-box-img" src="'+src+'"/>');
				jnBox.modal('show');
			}
		});

		jnBox.on('show', function(event){
			modalBody.find('.jn-gallery-box-img').unbind('load');
		...
		jnBox.on('hidden', function(event){
			modalBody.html(loadingHtml);
			jnBox.css('top', 'auto');
			$('.jn-gallery-box-img').unbind('load');
		});
